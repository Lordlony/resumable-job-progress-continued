...
//											ingredients.Add(job.targetB.Thing.def);
						...
						else if (OtherModCompatibility.IsMedievalOverhaulMendingDriver(driver))
						{
							RecipeDef recipe = job.RecipeDef;
							Thing repairThing = job.targetB.Thing;
							if (recipe is not null && repairThing is not null && !recipe.UsesUnfinishedThing)
							{
								float workLeft = Traverse.Create(driver).Field<float>("workLeft").Value;
								if (workLeft > 0f)
								{
									ResumeDictionary.Instance.SetResumeWorkLeft(repairThing, recipe, workLeft);

									// Medieval Overhaul awards non-unfinished mending XP at completion.
									// Settle the interrupted segment now so resumed work does not lose that XP.
									if (recipe.workSkill is not null)
									{
										int ticksSpent = Traverse.Create(driver).Field<int>("ticksSpentDoingRecipeWork").Value;
										float xp = ticksSpent * 0.1f * recipe.workSkillLearnFactor;
										pawn.skills?.GetSkill(recipe.workSkill).Learn(xp);
									}
								}
							}
...
// Guard: only handle plant targets to avoid NREs when interrupting non-plant jobs
if (job.targetA.Thing is Plant plant)
{
	// Use uninstallWorkLeft (public) rather than private workLeft to avoid reflection on non-public fields and to read the accumulated work safely.
	if (driverPlantWork.uninstallWorkLeft > 0)    // Use uninstallWorkLeft instead of workLeft, because uninstallWorkLeft is public.
	{
		float efficiency = 0f;
		// Only compute efficiency when there's an actual harvested thing; some plants don't yield harvest items.
		if (plant.def.plant.harvestedThingDef is not null)
		{
			StatDef statDef = plant.def.plant.harvestedThingDef.IsDrug ? StatDefOf.DrugHarvestYield : StatDefOf.PlantHarvestYield;
			efficiency = pawn.GetStatValue(statDef);
		}
		// Store the work-done snapshot so another pawn can resume this plant job later.
...
//						workDone = ResumeDictionary.Instance.GetResumeWorkDone(plant, job.def, out _);
						// Restore any previously saved work-done for this plant so the progress bar resumes where it left off.
...
//						ref float workDone = ref AccessTools.FieldRefAccess<JobDriver_PlantWork, float>(instance, "workDone");
						...
						if (__instance.uninstallWorkLeft >= property.harvestWork)
						{
							...
							{
								// No harvested product for this plant type — clear any leftover resume entries so they don't linger as stale state.
								_ = ResumeDictionary.Instance.PopResumeWorkDone(plant, job.def, out _, out _, property.harvestWork);
							}
...
//							plant.PlantCollected(actor, PlantDestructionMode.Smash);
				...
				else if (__instance is JobDriver_PlantSow)
				{
					___toils[1].initAction = Delegate.Combine(___toils[1].initAction, new Action(() =>
					{
						...
						{
							Traverse.Create(__instance).Field("sowWorkDone").SetValue(sowWorkDone);
						}
